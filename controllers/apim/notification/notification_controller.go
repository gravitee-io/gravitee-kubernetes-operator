...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	_, err := util.CreateOrUpdate(ctx, r.Client, notification, func() error {
		...
		if err := template.Compile(ctx, deepCopy, true); err != nil {
			return err
		}
	...
	if notification.Status.Conditions == nil {
		c := make([]v1.Condition, 0)
		notification.Status.Conditions = &c
	}

	if !notification.IsBeingDeleted() {
		err := r.setStatus(ctx, err, notification)
		// status update failed, retry later
		if err != nil {
...
func (r *Reconciler) setStatus(ctx context.Context, err error, notification *v1alpha1.Notification) error {
	if err != nil {
		meta.SetStatusCondition(notification.Status.Conditions, v1.Condition{
			Type:    conditionType,
