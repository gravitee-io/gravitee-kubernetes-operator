...
func setNotFoundTemplate(ctx context.Context, opts *mapper.Opts) {
	...
	if err := cli.Get(ctx, types.NamespacedName{Namespace: ns, Name: name}, &cm); err != nil {
		log.Error(ctx, err, "unable to access config map, using default HTTP not found template")
		return
	}

	if err := checkData(cm.Data); err != nil {
		log.Error(ctx, err, "missing key in config map, using default HTTP not found template")
		return
	}
