...
// WatchPortalThemes can be used to trigger a reconciliation when a PortalTheme is
// created or updated on resources that reference it (e.g. Portals), so that a portal
// waiting on a missing theme is synced as soon as the theme lands.
func (w *Type) WatchPortalThemes(index search.IndexField) *handler.Funcs {
	return &handler.Funcs{
		CreateFunc: w.CreateFromLookup(index),
		UpdateFunc: w.UpdateFromLookup(index),
	}
