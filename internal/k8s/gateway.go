...
func getVolumes(
	...
	volumes = append(volumes, getCAVolumes(gw)...)
...
func caVolumeName(listenerName gwAPIv1.SectionName) string {
	return "ca-" + string(listenerName)
}

func getCAVolumes(gw *gateway.Gateway) []coreV1.Volume {
	var volumes []coreV1.Volume
	seen := make(map[string]bool)
	...
	for i, l := range gw.Object.Spec.Listeners {
		...
		validation := getFrontendValidationForPort(gw.Object, l.Port)
		if validation == nil || len(validation.CACertificateRefs) == 0 {
			continue
		}
		volName := caVolumeName(l.Name)
		if seen[volName] {
			continue
		}
		seen[volName] = true
...
func getCAVolumeMounts(gw *gateway.Gateway) []coreV1.VolumeMount {
	var mounts []coreV1.VolumeMount
	seen := make(map[string]bool)
	...
	for i, l := range gw.Object.Spec.Listeners {
		...
		mounts = append(mounts, coreV1.VolumeMount{
			Name:      volName,
			MountPath: DefaultGatewayCAPath + string(l.Name),
			ReadOnly:  true,
		})
	}
	return mounts
...
func getVolumeMounts(
	...
	vm = append(vm, getCAVolumeMounts(gw)...)
...
func buildTLSForPort(
	...
	if validation := getFrontendValidationForPort(gw.Object, port); validation != nil {
		ssl["clientAuth"] = "required"
		truststore := make(map[string]any)
		truststore["type"] = "pem"
		truststore["path"] = buildCAPath(gw.Object, port)
...
func getFrontendValidationForPort(gw *gwAPIv1.Gateway, port gwAPIv1.PortNumber) *gwAPIv1.FrontendTLSValidation {
...
func buildCAPath(gw *gwAPIv1.Gateway, port gwAPIv1.PortNumber) string {
	for _, l := range gw.Spec.Listeners {
		if l.Port == port && l.Protocol == gwAPIv1.HTTPSProtocolType {
			return DefaultGatewayCAPath + string(l.Name) + "/ca.crt"
		}
	}
	return DefaultGatewayCAPath + "default/ca.crt"
}
