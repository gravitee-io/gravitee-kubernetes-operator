...
func ValidateDriftWithContext(
	...
	if !isDriftDetectionEnabled(newCRD) {
		return errs
	}
...
func isDriftDetectionEnabled(newCRD runtime.Object) bool {
	if coreObj, ok := newCRD.(core.Object); ok {
		...
		log.Panicf("CRD Does not implement core.Object interface: %T", newCRD)
	}
