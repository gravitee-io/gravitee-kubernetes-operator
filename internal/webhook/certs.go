...
func (p *Patcher) UpdateValidationCaBundle(ctx context.Context, secretName, ns string) error {
	webhookConfig, err := p.client.AdmissionregistrationV1().
		ValidatingWebhookConfigurations().Get(ctx, validatingWebhookName, metav1.GetOptions{})
...
func (p *Patcher) UpdateMutationCaBundle(ctx context.Context, secretName, ns string) error {
	webhookConfig, err := p.client.AdmissionregistrationV1().
		MutatingWebhookConfigurations().Get(ctx, mutatingWebhookName, metav1.GetOptions{})
