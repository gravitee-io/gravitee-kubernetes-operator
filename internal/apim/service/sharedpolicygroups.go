...
func (svc *SharedPolicyGroup) Search(query string, status string) ([]model.SharedPolicyGroup, error) {
	url := svc.EnvV2Target(sharedPolicyGroupsPath).WithQueryParam("q", query).WithQueryParam("status", status)
	sgp := new([]model.SharedPolicyGroup)

	if err := svc.HTTP.Get(url.String(), sgp); err != nil {
		return nil, err
	}
