...
- Envtest suites live under `test/envtest/<area>/`: controllers and admission against a real kube-apiserver + etcd with in-process mocks (AM today), run with `make envtest`
- Add unit and envtest suites only. Anything needing a cluster or a live APIM/AM belongs in the [platform e2e repo](https://github.com/gravitee-io/gravitee-platform-e2e); do not extend `test/integration/`
