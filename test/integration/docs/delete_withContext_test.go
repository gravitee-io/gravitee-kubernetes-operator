...
var _ = Describe("Delete", labels.WithContext, func() {
	...
	It("should delete documentation in APIM", func() {
		...
		Expect(manager.Client().Delete(ctx, fixtures.Documentation.DeepCopy())).To(Succeed())
		...
		By("calling rest API, expecting not to find documentation")

		Eventually(func() error {
			_, docErr := apimClient.Documentations.GetByHRID(parent, docHrid)
			return assert.NotFoundError(docErr)
		}, timeout, interval).Should(Succeed(), fixtures.Documentation.Name)
	})
})
