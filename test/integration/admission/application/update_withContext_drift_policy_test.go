...
const unreachableAPIMURL = "http://127.0.0.1:1"
...
var _ = Describe("Validate drift policies", labels.WithContext, func() {
	...
	It("should warn and allow the update when the remote is missing and policy is warn", func() {
		...
		patchContextBaseURL(ctx, fixtures.Context, constants.GatewayUrlHTTP)
	...
	It("should allow the update with no admission output when the remote is missing and policy is allow", func() {
		...
		waitUntilAdmissionErrorContains(ctx, admissionCtrl, fixtures.Application, newApp, "not found during drift detection")
	...
	It("should warn and allow the update when fetching the remote fails and policy is warn", func() {
		fixtures := applyMinimalApplication()
		env.Config.HTTPClientTimeoutSeconds = 1
		env.Config.DriftDetection.FetchFailurePolicy = env.DriftPolicyWarn
		patchContextBaseURL(ctx, fixtures.Context, unreachableAPIMURL)
		...
		Eventually(func() error {
			...
			return contains(warnings[0], "failed to fetch remote")
		}, constants.EventualTimeout, constants.Interval).Should(Succeed())
	})

	It("should allow the update with no admission output when fetching the remote fails and policy is allow", func() {
		fixtures := applyMinimalApplication()
		env.Config.HTTPClientTimeoutSeconds = 1
		patchContextBaseURL(ctx, fixtures.Context, unreachableAPIMURL)
		newApp := fixtures.Application.DeepCopy()
		...
		waitUntilAdmissionErrorContains(ctx, admissionCtrl, fixtures.Application, newApp, "failed to fetch remote")

		env.Config.DriftDetection.FetchFailurePolicy = env.DriftPolicyAllow
		warnings, err := admissionCtrl.ValidateUpdate(ctx, fixtures.Application, newApp)
		Expect(err).ToNot(HaveOccurred())
		Expect(warnings).To(BeEmpty())
	})
...
func patchContextBaseURL(ctx context.Context, mctx *v1alpha1.ManagementContext, baseURL string) {
	GinkgoHelper()
	Expect(manager.GetLatest(ctx, mctx)).To(Succeed())
	mctx.Spec.BaseUrl = baseURL
	Expect(manager.UpdateSafely(ctx, mctx)).To(Succeed())
}

func waitUntilAdmissionErrorContains(
	ctx context.Context,
	admissionCtrl admission.AdmissionCtrl,
	...
	Eventually(func() error {
		...
		if err == nil {
			return fmt.Errorf("expected admission error containing %q, got nil", substring)
		}
