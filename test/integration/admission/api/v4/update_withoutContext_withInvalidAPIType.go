...
var _ = Describe("Validate update", labels.WithContext, func() {
	...
	DescribeTable("should return error on api update with invalid Api Type",
		...
		Entry("should throw errors with wrong API Type",
			...
			apiV4.ApiType("PROXY"),
			"it is not possible to change the API Type. Current API type 'NATIVE', new API type 'PROXY'",
		),
		Entry("should throw errors with with wrong API Type",
			constants.ApiV4,
			apiV4.ApiType("NATIVE"),
			"it is not possible to change the API Type. Current API type 'PROXY', new API type 'NATIVE'",
		),
		Entry("should throw errors with with wrong API Type",
			constants.ApiV4,
			apiV4.ApiType("MESSAGE"),
			"it is not possible to change the API Type. Current API type 'PROXY', new API type 'MESSAGE'",
		),
	)
})
