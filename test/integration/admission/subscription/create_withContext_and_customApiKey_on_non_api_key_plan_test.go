...
var _ = Describe("Validate create", labels.WithContext, func() {
	...
	fixtures.Subscription.Spec.CustomApiKey = "my-custom-key"
	fixtures.Subscription.Namespace = constants.Namespace

	fixtures.Apply()

	It("should fail if customApiKey is set on non API_KEY plan", func() {
		Eventually(func() error {
			...
			return assert.Equals(
				"error",
				errors.NewSeveref(
					"customApiKey can only be set when subscribing to an API_KEY plan, got [JWT]",
				),
				err,
			)
