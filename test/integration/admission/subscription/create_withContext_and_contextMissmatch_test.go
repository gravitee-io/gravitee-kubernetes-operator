...
var _ = Describe("Validate create", labels.WithContext, func() {
	...
	Expect(k8s.GetClient().Create(ctx, appCtx)).To(Succeed())

	fixtures.Application.Spec.Context.Name = appCtx.Name

	Expect(manager.UpdateSafely(ctx, fixtures.Application)).To(Succeed())

	It("should fail if contexts do not match", func() {
