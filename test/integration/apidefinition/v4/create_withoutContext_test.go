...
var _ = Describe("Create", labels.WithoutContext, func() {
	...
	DescribeTable("without a management context",
		...
		Entry(
			...
			fixture.Builder().WithAPIv4(constants.ApiV4),
			200,
		),
		Entry(
			"should resolve the template and make api available",
			fixture.Builder().WithAPIv4(constants.ApiV4WithTemplatingFile),
			200,
		),
		Entry(
			"should make api with rate limit available",
			fixture.Builder().WithAPIv4(constants.ApiV4WithRateLimit),
			200,
		),
		Entry(
			"should make api with disabled policy available",
			fixture.Builder().WithAPIv4(constants.ApiV4WithDisabledPolicy),
			200,
		),
	)
})
