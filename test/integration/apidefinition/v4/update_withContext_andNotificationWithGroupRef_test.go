...
var _ = Describe("Update", labels.WithContext, func() {
	...
	It("should update notification events after creation", func() {
		...
		fixtures.Notification.Spec.Console.APIEvents = []string{"API_STARTED", "API_STOPPED", "APIKEY_EXPIRED"}
		...
		Eventually(func() error {
			...
			if err := assert.Equals("console.Hooks",
				[]string{"API_STARTED", "API_STOPPED", "APIKEY_EXPIRED"},
	...
	It("should delete notification after creation", func() {
		...
		Expect(manager.UpdateSafely(ctx, fixtures.APIv4)).To(Succeed())
