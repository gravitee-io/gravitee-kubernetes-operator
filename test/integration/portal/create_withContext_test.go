...
var _ = Describe("Create", labels.WithContext, func() {
	...
	It("should create portal in APIM and persist navigation in list order", func() {
		...
		Eventually(func() error {
			...
			// APIM echoes only explicitly authored navigation entries — implicit
			// intermediate folders are not materialised in GET /portals/{hrid}.
			// The assertion is strict equality because the round-trip is lossless.
			return assert.Equals("Portal navigation", expectedPaths, paths)
