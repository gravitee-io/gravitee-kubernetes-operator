...
var _ = Describe("Creating an Application", func() {
	httpClient := http.Client{Timeout: 5 * time.Second}

	Context("Create Application", func() {
		...
		It("Should not create Application without context", func() {
...
func basicAuth(mc *gio.ManagementContext) string {
	auth := mc.Spec.Auth.Credentials.Username + ":" + mc.Spec.Auth.Credentials.Password
