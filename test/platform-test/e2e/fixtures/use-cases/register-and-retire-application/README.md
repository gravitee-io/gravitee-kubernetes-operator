...
An API consumer registers an application, edits it, then retires it. The same
journey runs against both provisioners and asserts the same APIM outcome.
...
# one driver
npm --prefix test/platform-test run e2e -- --grep @GKO-335 --provision-with gko
