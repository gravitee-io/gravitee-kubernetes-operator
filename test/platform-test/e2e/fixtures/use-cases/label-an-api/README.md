...
Organise a V4 API with labels, then strip them. Labels are an **inline attribute**
of the API (`apim_apiv4.labels`) — there is no standalone resource, but the
journey is still fully expressible through both provisioners. The same intent
runs against both.
