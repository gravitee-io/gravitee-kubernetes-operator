...
test.describe(`V2 API Categories — Extended ${PROVISIONER.GKO}`, () => {
  test.afterEach(async () => {
    await kubectlSafe
      .del(fixture("categories/v2-with/crd.yaml"))
      .catch(() => {});
    await kubectlSafe
      .del(fixture("categories/v2-without/crd.yaml"))
      .catch(() => {});
    await kubectlSafe
      .del(fixture("categories/v2-with-many/crd.yaml"))
      .catch(() => {});
    await kubectlSafe
      .del(fixture("categories/v2-renamed/crd.yaml"))
      .catch(() => {});
    await kubectlSafe
      .del(fixture("categories/v2-non-existing/crd.yaml"))
      .catch(() => {});
  });
  ...
  test(`Create V2 API with a category ${XRAY.CATEGORIES.V2_VALID_CATEGORY} ${TAGS.REGRESSION}`, async ({
    ...
    const fixturePath = fixture("categories/v2-with/crd.yaml");
  ...
  test(`Deploy V2 API with many category refs ${XRAY.CATEGORIES.V2_MANY_CATEGORIES} ${TAGS.REGRESSION}`, async ({
    ...
    const fixturePath = fixture("categories/v2-with-many/crd.yaml");
  ...
  test(`V2 API with non-existing category is accepted ${XRAY.CATEGORIES.V2_NON_EXISTING_CATEGORY} ${TAGS.REGRESSION}`, async ({
    ...
    const fixturePath = fixture("categories/v2-non-existing/crd.yaml");
  ...
  test(`Remove category from V2 API on re-apply ${XRAY.CATEGORIES.V2_REMOVE_CATEGORY} ${TAGS.REGRESSION}`, async ({
    ...
    const API_NAME = "e2e-v2-with-category";
    const withCategory = fixture("categories/v2-with/crd.yaml");
    const withoutCategory = fixture("categories/v2-without/crd.yaml");
  ...
  test(`Category rename on V2 API triggers redeploy ${XRAY.CATEGORIES.V2_CATEGORY_RENAME_REDEPLOY} ${TAGS.REGRESSION}`, async ({
    ...
    const API_NAME = "e2e-v2-many-cats";
    const original = fixture("categories/v2-with-many/crd.yaml");
    const renamed = fixture("categories/v2-renamed/crd.yaml");
