...
/**
 * Journey: reuse a shared policy group across an API.
 *
 * As an API producer, I define a shared policy group once and reuse it across an
 * API. Currently BLOCKED on both provisioners, so both arms are `pending` (a
 * visible gap, never a green-washed test). The intended invariant is
 * provisioner-agnostic: an API whose request flow invokes a Shared Policy Group
 * runs that SPG at the gateway, and detaching it stops it.
 *
 * Fixtures are co-located in this folder. Blockers (see the co-located README.md):
