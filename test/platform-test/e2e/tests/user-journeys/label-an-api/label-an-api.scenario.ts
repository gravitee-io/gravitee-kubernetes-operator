 ...
 *
 * As an API producer, I organise my API with portal labels. Labels are an inline
 * attribute of apim_apiv4 (no standalone Terraform resource), yet the journey is
 * fully expressible through both provisioners: labels set through either driver
 * land in APIM and are removed when stripped. This is the reference pattern for
 * the other inline apim_apiv4 attributes (categories, groups, metadata, inline
 * pages[]) that have no standalone Terraform resource.
 *
...
forEachProvisioner<LabelParams>(
  {
    ...
    provisioners: {
      gko: gkoScenario<LabelParams>({
        manifests: [],
        roles: { api: "labeled-api" },
        dynamicRoles: ["api"],
        applyParams: async (k, params) => {
          await k.apply(path.join(here, params.withLabels ? "gko/api-with-labels.yaml" : "gko/api-without-labels.yaml"));
        },
      }),
