 ...
 *
 * Deliberately the same test as ../../playwright/tests/provision-and-assert.test.ts,
 * differing only in the runner's hook/assertion names — that IS the demonstration:
 * `GkoProvisioner` and the `mapi` / `gateway` assertions depend only on `node:*`,
 * `yaml` and native `fetch`, so nothing in the library ties them to a runner.
 *
 * Requires a cluster with APIM + the GKO operator:
 *
 *   MANIFEST=/path/to/api.yaml API_NAME=my-api npx jest provision-and-assert
 */
...
describe("Provision through GKO, assert against APIM", () => {
  it("starts the API in APIM and serves traffic", async () => {
