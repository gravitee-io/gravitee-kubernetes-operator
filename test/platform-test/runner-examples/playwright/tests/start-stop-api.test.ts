...
test.describe("Start / Stop API lifecycle", () => {
  test("should respond 200 when the API is started", async () => {
    await mapi.assertApiStarted(API_ID);
    await gateway.assertResponds(API_PATH, { status: 200 });
  });

  test("should respond 404 when the API is stopped", async () => {
    await mapi.assertApiStopped(API_ID);
    await gateway.assertResponds(API_PATH, { status: 404 });
  });

  test("should respond 200 again when the API is re-started", async () => {
    await mapi.assertApiStarted(API_ID);
    await gateway.assertResponds(API_PATH, { status: 200 });
  });
});
