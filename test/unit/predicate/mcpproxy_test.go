...
func mcpProxy(contextPath string) *v1alpha1.McpProxy {
	return &v1alpha1.McpProxy{
		...
		Spec: v1alpha1.McpProxySpec{
			Type: mcpproxy.Type{
				...
				Proxy:       &mcpproxy.Proxy{ServerURL: "https://api.githubcopilot.com/mcp/"},
			},
		},
	}
}

var _ = Describe("LastSpecHashPredicate McpProxy", func() {
	p := predicate.LastSpecHashPredicate{}

	It("reconciles a create with no last-spec-hash", func() {
		Expect(p.Create(event.CreateEvent{Object: mcpProxy("/mcp/github")})).To(BeTrue())
	})

	It("skips a create whose last-spec-hash already matches the spec", func() {
		obj := mcpProxy("/mcp/github")
	...
	It("reconciles an update when the spec changes", func() {
		oldObj := mcpProxy("/mcp/github")
		newObj := mcpProxy("/mcp/other")
